...
%% Constraint 58
Xg58 = eye(len.Xg);
Aeq58 = zeros(len.Xg,len.total);
Aeq58(:,inp.Xg) = Xg58;
Aeq58 = Aeq58(data.statgen == 0,:);
beq58 = zeros(size(Aeq58,1),1);

equ(58).Aeq = concA(steps,Aeq58);
equ(58).beq = concB(steps,beq58);
...
%% Constraint 60 
% Sn60 = eye(len.Sn);
% Aeq60 = zeros(len.Sn,len.total);
% Aeq60(:,inp.Sn) = Sn60;
% Aeq60 = Aeq60(data.statbus == 0,:);
% beq60 = zeros(size(Aeq60,1),1);
% 
% equ(60).Aeq = concA(steps,Aeq60);
% equ(60).beq = concB(steps,beq60);
...
%% Constraint 5 (( U_ijt - U_j - 2.r_ij.P_brt - 2.x_ij.Qbrt + M.Xbrt <= M ))
Ut5 = zeros(len.Pbr,len.Sn);
Pbr5 = zeros(len.Pbr); Qbr5 = Pbr5;
Xbr5 = zeros(len.Xbr);
for i = 1:len.Pbr
    Ut5(i,data.branch(i,2)) = 1; % U_it
    Ut5(i,data.branch(i,3)) = -1; % U_jt
    Pbr5(i,i) = -2 * data.branch(i,6); % - 2.r_ij.P_bsrt
    Qbr5(i,i) = -2 * data.branch(i,7); % - 2.x_ij.Qbrt
    Xbr5(i,i) = bigM; % M.Xbrt
end
A5 = zeros(size(Ut5,1),len.total);
A5(:,(inp.U)) = Ut5;
A5(:,(inp.Pbr)) = Pbr5;
A5(:,(inp.Qbr)) = Qbr5;
A5(:,(inp.Xbr)) = Xbr5;
b5 = ones(size(Ut5,1),1) .* bigM;
ineq(5).A = concA(steps,A5);
ineq(5).b = concB(steps,b5);

%% Constraint 6 (( - U_ijt + U_j + 2.r_ij.P_brt + 2.x_ij.Qbrt - M.Xbrt <= M ))
Ut6 = -Ut5; Pbr6 = -Pbr5; Qbr6 = -Qbr5; Xbr6 = -Xbr5;
A6 = zeros(size(Ut6,1),len.total);
A6(:,(inp.U)) = Ut6;
A6(:,(inp.Pbr)) = Pbr6;
A6(:,(inp.Qbr)) = Qbr6;
A6(:,(inp.Xbr)) = Xbr6;
...
%% Constraint 9 and 10 >> Skipped
Plt9 = eye(len.Pl); Xl9 = -eye(len.Xl) .* data.load(:,4);
Qlt10 = eye(len.Ql); Xl10 = -eye(len.Xl) .* data.load(:,5);

Aeq9 = zeros(size(Plt9,1),len.total);
Aeq9(:,(inp.Pl)) = Plt9;
Aeq9(:,inp.Xl) = Xl9;

Aeq10 = zeros(size(Qlt10,1),len.total);
Aeq10(:,(inp.Ql)) = Qlt10;
Aeq10(:,inp.Xl) = Xl10;
...
% C.17 -- A
Pgt17a = -eye(len.Pg);
Xgt17a = eye(len.Xg) .* data.gen(:,7);
A17a = zeros(size(Pgt17a,1),len.total);
A17a(:,(inp.Pg)) = Pgt17a;
A17a(:,(inp.Xg)) = Xgt17a;
b17a = zeros(len.Xg,1);
...
% C.17 -- B
Pgt17b = -Pgt17a;
Xgt17b = -eye(len.Xg) .* data.gen(:,6);
A17b = zeros(size(Pgt17b,1),len.total);
A17b(:,(inp.Pg)) = Pgt17b;
A17b(:,(inp.Xg)) = Xgt17b;
b17b = b17a;
...
% C.18 -- A
Qgt18a = Pgt17a;
Xgt18a = eye(len.Xg) .* data.gen(:,9);
A18a = zeros(size(Qgt18a,1),len.total);
A18a(:,(inp.Qg)) = Qgt18a;
A18a(:,(inp.Xg)) = Xgt18a;
b18a = b17a;
...
% C.18 -- B
Qgt18b = -Qgt18a;
Xgt18b = -eye(len.Xg) .* data.gen(:,8);
A18b = zeros(size(Qgt18b,1),len.total);
A18b(:,(inp.Qg)) = Qgt18b;
A18b(:,(inp.Xg)) = Xgt18b;
b18b = b18a;
...
for i = 1:len.Pg
    if data.gen(i,10) ~= 1
        Pgt19(i,i) = tan(acos(data.gen(i,4)));
        Qgt19(i,i) = -1;
    end
end

Aeq19 = zeros(size(Pgt19,1),len.total);
Aeq19(:,(inp.Pg)) = Pgt19;
Aeq19(:,(inp.Qg)) = Qgt19;
Aeq19 = Aeq19(data.statgen == 2,:);
...
A23b = zeros(len.Pessd,len.total);
...
for i = 1:len.Xg % Fills corresponding node that contains generator
    Sn40(i,data.gen(i,1)) = -1;
end

A40 = zeros(len.Xg,len.total);
A40(:,inp.Xg) = Xg40;
A40(:,inp.Sn) = Sn40;
A40 = A40(data.statgen==2,:); % Remove the substation and faulty generator
...
for i = 1:len.Xl
    Sn46(i,data.load(i,1)) = -1;
end
Aeq46 = zeros(len.Xl,len.total);
Aeq46(:,inp.Xl) = Xl46;
Aeq46(:,inp.Sn) = Sn46;
Aeq46 = Aeq46(data.statload == 1,:);
...
A49(:,inp.Xl) = Xl49;
A49 = A49(data.statload == 2,:);

A49_bef = zeros(len.Xl,len.total);
A49_bef(:,inp.Xl) = Xl49_bef;
A49_bef = A49_bef(data.statload == 2,:);
...
%% Dumm
% SnX = eye(len.Sn);
% AeqX = zeros(len.Sn,len.total);
% AeqX(:,inp.Sn) = SnX;
% 
% beqX = zeros(size(AeqX,1),1);
% 
% AeqX = initials(steps,AeqX);
% equ(62).Aeq = AeqX;
% equ(62).beq = beqX;
